...
class WisskiPathForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $pb = WisskiPathbuilderEntity::load($this->pb);
    ...
    if ($this->operation == 'edit') {
      $form['#title'] = $this->t('Edit Path: @id', ['@id' => $path->getID()]);
    }
    ...
    if ($engine->providesSimpleAlternatives()) {
      ...
      if ($trigger = $form_state->getTriggeringElement()) {
        if ($trigger['#parents'][0] === 'mode_button') {
          $simple_mode = !$simple_mode;
        }
      }
      $form['mode_selection'] = [
        '#type' => 'details',
        '#title' => $this->t('Step Detection Mode: %mode', ['%mode' => ($simple_mode ? $this->t('simple') : $this->t('complex'))]),
    ...
    if (empty($input)) {
      ...
      $paout = isset($storage['existing_paths']) ? $storage['existing_paths'] : [];
      // dpm($storage,'Click');
      ...
      if (!empty($paout)) {
        ...
        if ($trigger_type === 'select') {
          ...
          // $input['path_array']['step:'.$row_num]['select'];.
          $paout[$row_num] = $input['wisskipathselect' . $row_num];
        }
      ...
      // Case else - primary if we are editing.
      else {
    ...
    $form['path_array'] = [
      ...
      '#suffix' => '</div>',
      '#header' => ['step' => $this->t('Step'), 'edit' => '', 'opis' => ''],
    // '#tree' => TRUE,.
    ];
    // dpm($curvalues, "curval");
    // go through all values and create fields for them.
    foreach ($curvalues as $key => $element) {
      $form['path_array']['step:' . $key] = [
        '#type' => 'container',
        '#tree' => TRUE,
        '#attributes' => ['class' => 'wisski-row', 'id' => 'wisski-row-' . $key],
      ];
      // drupal_set_message("key " . $key . ": element " . $element);.
      if ($key > 0) {
        $pre = $curvalues[($key - 1)] !== 'empty' ? [$curvalues[($key - 1)]] : [];
      ...
      $form['path_array']['step:' . $key]['select' . $key] = [
      // '#default_value' => 'empty',.
        '#value' => $element,
        '#type' => 'select',
      ...
      // '#title_display' => 'invisible',.
        '#attributes' => ['data-wisski' => 'select' . $key],
        '#description' => $pre,
        '#ajax' => [
      ...
      $form['path_array']['step:' . $key]['edit_btn' . $key] = [
        '#type' => 'link',
        '#title' => $this->t('edit'),
    ...
    $form['path_array']['datatype_property'] = [
      '#type' => 'container',
      '#tree' => TRUE,
    ];
    // dpm($form);
    ...
    if (count($curvalues) > 1 && count($curvalues) % 2 == 0 && $primitive = $engine->getPrimitiveMapping($curvalues[(count($curvalues) - 2)])) {
      if (count($primitive) == 1) {
        ...
        // dpm($default,'Default');.
        $form_state->setValue(['path_array', 'datatype_property', 'datatypeselect'], $default);
        $form['path_array']['datatype_property']['datatypeselect'] = [
    ...
    $form['path_array']['datatype_property']['opis'] = [
      '#type' => 'hidden',
      '#value' => 'opis',
    ];
    ...
    if (count($curvalues) > 1 && count($curvalues) % 2 == 0) {
      ...
      $form['path_array']['disamb']['disambselect'] = [
        '#value' => $disamb,
        '#type' => 'select',
        '#empty_value' => 'empty',
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status) {
      ...
      drupal_set_message(
        $this->t(
            'The path @id could not be saved.', [
              '@id' => $path->getID(),
            ]
        )
        );
    ...
    $form_state->setRedirect('entity.wisski_pathbuilder.edit_form', ['wisski_pathbuilder' => $pbid]);
  }
