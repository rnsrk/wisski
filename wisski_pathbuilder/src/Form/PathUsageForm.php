...
class PathUsageForm extends FormBase {
  ...
  public function deleteChecked($form, FormStateInterface $form_state) {
    ...
    drupal_set_message($this->t('@c paths have been deleted.', ['@c' => $cnt_orph]));
    drupal_set_message($this->t('@c paths have been removed from their pathbuilders.', ['@c' => $cnt_semi]));
    if ($cnt_semi) {
      drupal_set_message($this->t('There may be paths that have become orphaned as they have been removed from their pathbuilders.'));
    }
