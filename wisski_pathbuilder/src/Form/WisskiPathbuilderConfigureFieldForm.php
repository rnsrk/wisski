...
class WisskiPathbuilderConfigureFieldForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if ($path->getType() != "Path") {
      ...
      // dpm($default_value);
      // return $form;.
      $form['choose_bundle']['select_bundle'] = [
        '#type' => 'select',
    ...
    if ($path->getType() == "Path") {
      ...
      if (!empty($bundle_id)) {
        ...
        $bundle_fields = \Drupal::entityManager()->getStorage('field_config')->loadByProperties(['bundle' => $bundle_id]);
      ...
      $form['field_form']['choose_field'] = [
        '#type' => 'fieldset',
        '#title' => $this->t('Choose field'),
      ];

      $form['field_form']['choose_field']['select_field'] = [
        '#type' => 'select',
        '#description' => (!empty($bundle_id) ? $this->t('Select an existing field from bundle %bundle_label', ['%bundle_label' => $bundle_label . ' (' . $bundle_id . ')']) : $this->t('Should a field be created?')),
      ...
      $form['field_form']['choose_field']['field'] = [
        '#type' => 'textfield',
        '#maxlength' => 255,
        ...
        if (isset($bundle_fields[$selected_field_name])) {
          ...
          $selected_field_values = [
            ...
            'formatter' => \Drupal::entityManager()
              ...
              ->getComponent($selected_field_name)['type'],
      ...
      if (empty($ftvalue) && isset($selected_field_values)) {
        // $form_state->getValue('fieldtype');.
        $ftvalue = $selected_field_values['field_type'];
      }
      ...
      $display['field_display']['displaywidget'] = [
        '#type' => 'select',
        '#maxlength' => 255,
      ...
      if (isset($selected_field_values['widget'])) {
        $display['displaywidget']['#value'] = $selected_field_values['widget'];
      }

      $display['field_display']['formatterwidget'] = [
        '#type' => 'select',
        '#maxlength' => 255,
      ...
      if (isset($selected_field_values['formatter'])) {
        $display['formatterwidget']['#value'] = $selected_field_values['formatter'];
      }
      ...
      if (isset($selected_field_values['cardinality'])) {
        $display['cardinality']['#value'] = $selected_field_values['cardinality'];
      }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    $form_state->setRedirect('entity.wisski_pathbuilder.edit_form', ['wisski_pathbuilder' => $this->pathbuilder->id()]);
