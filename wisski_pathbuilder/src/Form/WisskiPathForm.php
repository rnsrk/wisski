...
class WisskiPathForm extends EntityForm
{
        ...
        if ($trigger = $form_state->getTriggeringElement()) {
            ...
            case 'operations': {
                ...
                case 'change': $selected_row = $row_selection; 
                    break;
                case 'consistent_change': $consistent_change = true; $selected_row = $row_selection; 
                    break;
                case 'remove': $this->path_array = WisskiHelper::array_remove_part($this->path_array, $row_selection, 2); 
                    break;
                case 'insert': $this->path_array = WisskiHelper::array_insert($this->path_array, array('empty','empty'), $row_selection+1); 
                    break;
                }
                break;
}
    ...
    public function save(array $form, FormStateInterface $form_state)
    {
        ...
        if($status) {
            ...
            drupal_set_message(
                $this->t(
                    'The path @id could not be saved.', array(
                    '@id' => $path->getID(),
                    )
                ), 'error'
            );
