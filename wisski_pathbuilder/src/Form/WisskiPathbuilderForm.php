...
class WisskiPathbuilderForm extends EntityForm {
        ...
        if (!empty($pbpaths[$path->id()]) && !empty($pbpaths[$path->id()]['bundle'])) {
          ...
          // 'url' => $path->urlInfo('edit-form', array('wisski_pathbuilder'=>$pathbuilder->id())),
            'url' => Url::fromRoute('entity.wisski_bundle.edit_form')
              ->setRouteParameters(['wisski_bundle' => $pbpaths[$path->id()]['bundle']]),
          ];
          /*
    ...
    foreach ($xmldoc->path as $path) {
      ...
      // Normal case - import the path!
        ...
        if ($pathdata['name'] == '') {
          ...
          drupal_set_message(t('Path with id @id (@uuid) has no name. Name has been set to "_empty_".', ['@id' => $pathdata['id'], '@uuid' => $uuid]), 'warning');
        }
  ...
  private function pb_render_path($pathid, $enabled, $weight, $depth, $parent, $bundle, $field, $fieldtype, $displaywidget, $formatterwidget, $cardinality, $namespaces) {
    ...
    $pathform['title'] = [
      '#type' => 'label',
      '#title' =>
      $path->getName(), '#attributes' => $path->isGroup() ? ['style' => 'font-weight: bold;'] : ['style' => 'font-weight: normal; font-style:italic;'],
    ];
    ...
    if (!empty($paths)) {
      foreach ($paths as $key => $path) {
        ...
        if ($path['enabled'] == 0) {
          ...
          // Delete old fields.
          $field_storages = \Drupal::entityManager()->getStorage('field_storage_config')->loadByProperties(['field_name' => $field]);
          ...
          $field_objects = \Drupal::entityManager()->getStorage('field_config')->loadByProperties(['field_name' => $field]);
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status) {
      ...
      drupal_set_message(
        $this->t(
            'The Pathbuilder @id could not be saved.', [
              '@id' => $pathbuilder->id(),
            ]
        ), 'error'
        );
