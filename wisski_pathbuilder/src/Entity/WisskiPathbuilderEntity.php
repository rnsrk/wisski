...
class WisskiPathbuilderEntity extends ConfigEntityBase implements WisskiPathbuilderInterface {
  ...
  public function generateFieldForSubGroup($pathid, $field_name, $orig_bundle) {
    ...
    if (empty($field_name)) {
      drupal_set_message(t('No field name was provided for the field associated with path id %id. This is evil, please change.', ['%id' => $pathid]), "error");
      return;
    }
    ...
    if ($orig_bundle === self::CONNECT_NO_FIELD) {
      return;
    }

    // Create a new field if the user whishes to.
    if ($orig_bundle === self::GENERATE_NEW_FIELD || empty($bundle) || empty($orig_bundle)) {
      $fieldid = $this->generateIdForBundle($pathid);
    }
    // There already is a bundle.
      ...
      if ($field_storages = \Drupal::entityManager()->getStorage('field_storage_config')->loadByProperties(['field_name' => $fieldid])) {
        // drupal_set_message(serialize($field_storages));
        if (WISSKI_DEVEL) {
          drupal_set_message(t('Field %bundle with id %id was already there.', ['%bundle' => $field_name, '%id' => $fieldid]));
        }
        ...
        $field_values = \Drupal::entityManager()->getStorage('field_config')->loadByProperties(['field_name' => $fieldid, 'entity_type' => 'wisski_individual']);
    ...
    if (!$no_fs) {
      ...
      // Has to fit the field component type, see below.
        'type' => ($type == 'wisski_bundle') ? 'entity_reference' : 'field_collection',
        'translatable' => TRUE,
        'cardinality' => $card,
      ];
    ...
    if (!$no_fs) {
      if (WISSKI_DEVEL) {
        drupal_set_message(t('Created new field %field in bundle %bundle for this path', ['%field' => $field_name, '%bundle' => $bundle]));
      }
    }
    else {
    }
  ...
  public function generateFieldForPath($pathid, $field_name) {
    ...
    // Create everything as wisski individual things.
    else {
      $mode = 'wisski_individual';
    }
    ...
    // don't go on if the user whishes not to.
    if ($fieldid === self::CONNECT_NO_FIELD) {
      return;
    }

    // Create a new field if the user whishes to.
    if ($fieldid === self::GENERATE_NEW_FIELD || empty($fieldid)) {
      ...
      foreach ($bundle_infos as $target_bundle_id => $bundle_info) {
        foreach ($bundle_info as $pbid => $info) {
          if (isset($info['main_concept'][$main_concept])) {
            ...
            // 4. rest.
            $rank = 0;
            if (isset($info['is_top_concept'][$main_concept])) {
              $rank += 2;
            }
            if ($this->id() == $pbid) {
              $rank += 1;
    ...
    if (empty($field_name)) {
      drupal_set_message(t('Cannot create field for path %path_id without field name', ['5path_id' => $pathid]), 'error');
    }
    ...
    if (!empty($field_storages)) {
      if (count($field_storages) > 1) {
        drupal_set_message('There are multiple field storages for this field name: ' . $field_name, 'warning');
      }
      ...
      if ($field_storage->getType() != $field_storage_values['type']) {
        ...
        drupal_set_message(t('Field %bundle with id %id had to be deleted and recreated.', ['%bundle' => $field_name, '%id' => $fieldid]), 'warning');
        $create_fs = TRUE;
      }
    }
    else {
    ...
    if ($create_fs) {
      ...
      // If everything is there - why not skip? By Mark: I am unsure if this is a good idea.
    }
    else {
      // return;.
    }
    ...
    // 'text_summary_or_trimmed',//has to fit the field type, see above.
      'type' => $pbpaths[$pathid]['displaywidget'],
    ...
    if (strpos($pbpaths[$pathid]['fieldtype'], 'image') !== FALSE) {
      if (strpos($pbpaths[$pathid]['formatterwidget'], 'wisski_iip_image') !== FALSE) {
        $display_options = [
          ...
          'settings' => ['colorbox_node_style' => 'medium', 'colorbox_image_style' => 'large'],
          'weight' => $pbpaths[$pathid]['weight'],
        ];
      }
      else {
  ...
  public function generateBundleForGroup($groupid) {
    ...
    // don't go on if the user whishes not to.
    if ($my_group['bundle'] === self::CONNECT_NO_FIELD) {
      return;
    }
    ...
    if (!empty($my_group['bundle']) && $my_group['bundle'] !== self::GENERATE_NEW_FIELD) {
      ...
      $bundles = \Drupal::entityManager()->getStorage($mode)->loadByProperties(['id' => $bundleid]);

      if (!empty($bundles)) {
        $bundle = current($bundles);
        $bundle_name = $bundle->label();
        if (WISSKI_DEVEL) {
          drupal_set_message(t('Connected bundle %bundlelabel (%bundleid) with group %groupid.', ['%bundlelabel' => $bundle_name, '%bundleid' => $bundleid, '%groupid' => $groupid]));
        }
      }
      else {
        if (WISSKI_DEVEL) {
          drupal_set_message(t('Could not connect bundle with id %bundleid with group %groupid. Generating new one.', ['%bundleid' => $bundleid, '%groupid' => $groupid]));
        }
    ...
    if (empty($bundleid) || $regenerate) {
      ...
      if (empty($bundle_name) || !empty(\Drupal::entityManager()->getStorage($mode)->loadByProperties(['id' => $bundleid]))) {
        if (WISSKI_DEVEL) {
          drupal_set_message(t('Bundle %bundle with id %id was already there.', ['%bundle' => $bundle_name, '%id' => $bundleid]));
        }
      ...
      $bundle = \Drupal::entityManager()->getStorage($mode)->create(['id' => $bundleid, 'label' => $bundle_name]);
      $bundle->save();
      ...
      if (is_null($evd)) {
        $evd = \Drupal::entityManager()->getStorage('entity_view_display')->create($view_entity_values);
      }
      $efd = \Drupal::entityManager()->getStorage('entity_form_display')->load('wisski_individual.' . $bundleid . '.default');
      if (is_null($efd)) {
        $efd = \Drupal::entityManager()->getStorage('entity_form_display')->create($view_entity_values);
      }
      ...
      if (WISSKI_DEVEL) {
        drupal_set_message(t('Created new bundle %bundle for group with id %groupid.', ['%bundle' => $bundle_name, '%groupid' => $groupid]));
      }
    }
  ...
  public function addPathToPathTree($pathid, $parentid = 0, $is_group = FALSE) {
    ...
    if (empty($parentid)) {
      $pathtree[$pathid] = ['id' => $pathid, 'children' => []];
    }
    else {
      ...
      // and add it there.
      $pathtree = $this->addDataToParentInTree($parentid, ['id' => $pathid, 'children' => []], $pathtree);
    ...
    // }.
    if ($is_group) {
      $pbpaths[$pathid] = ['id' => $pathid, 'weight' => 0, 'enabled' => 0, 'parent' => $parentid, 'bundle' => '', 'field' => '', 'fieldtype' => '', 'displaywidget' => '', 'formatterwidget' => ''];
    }
    else {
      ...
      $pbpaths[$pathid] = ['id' => $pathid, 'weight' => 0, 'enabled' => 0, 'parent' => $parentid, 'bundle' => '', 'field' => self::CONNECT_NO_FIELD, 'fieldtype' => 'string', 'displaywidget' => 'string_textfield', 'formatterwidget' => 'string'];
    }
  ...
  public function getAllBundleIdsAboutUri($uri) {
          ...
          if ($last == $uri || $first == $uri) {
            if (!empty($group['parent'])) {
              if (empty($group['bundle'])) {
                ...
                $nontopbundles = array_merge($nontopbundles, [$group['bundle'] => $group['bundle']]);
              }

            }
            elseif (empty($group['bundle'])) {
              $p = entity_load('wisski_path', $group['id']);
              if (empty($p)) {
                drupal_set_message("Group " . $groupid . " does not exist anymore in PB " . $this->getName() . ". We remove it.", "warning");
                unset($pbpaths[$groupid]);
              ...
              $topbundles = array_merge($topbundles, [$group['bundle'] => $group['bundle']]);
            }
    ...
    return [$topbundles, $nontopbundles];
  ...
  public function getImagePathIDsForGroup($groupid, $recursive = TRUE, $subtree = NULL) {
    ...
    if (empty($subtree)) {
      ...
      while ($to_look && $to_look['parent']) {
        $parents = array_merge([$to_look['parent']], $parents);
  ...
  public function getRelativePath($path, $with_start_connection = TRUE) {
    ...
    // If not, we load it lateron, but first have a look in the cache.
    else {
