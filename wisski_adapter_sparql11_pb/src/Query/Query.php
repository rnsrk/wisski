...
class Query extends WisskiQueryBase {
  ...
  public function getQueryParts() {
    ...
    return ["where" => $where_clause, "eids" => $entity_ids];
  }
    ...
    foreach ($condition->conditions() as $ij => $cond) {
      ...
      if ($this->isPathQuery() && strpos($field, '.') !== FALSE) {
        ...
        if (count($pb_and_path) != 2) {
          ...
          drupal_set_message(new TranslatableMarkup('Bad pathbuilder and path id "%id" in entity query condition', ['%id' => $field]));
          // With next condition.
          continue;
        }
        $pbid = $pb_and_path[0];
  ...
  protected function makePathCondition($pb, $path, $operator, $value, $starting_group = NULL) {
    ...
    if ($operator == "EMPTY") {
      ...
      // If it has no dt we use the last element.
      else {
