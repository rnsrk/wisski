...
class Sparql11EngineWithPB extends Sparql11Engine implements PathbuilderEngineInterface {
        ...
        if (empty($main_property)) {
          ...
          if (is_null($disamb) == TRUE) {
            ...
            if (!isset($thing->{$disambname})) {
              ...
              $out[] = [$main_property => $outvalue, 'wisskiDisamb' => $thing->{$disambname}->dumpValue("text")];
            }
          }
        }
      }
  ...
  public function deleteOldFieldValue($entity_id, $fieldid, $value, $pb, $count = 0, $mainprop = FALSE) {
    ...
    if ($is_reference) {
      ...
      elseif (count($path_array) == 3) {
        ...
        // only do this for groups - for fields we have to handle this otherwise.
        if ($path->isGroup()) {
        ...
        // This is the case for the entity-reference fields that are not made by wisski.
        else {
          $subject_uris = [$subject_uri];
        }
   ...
   *   an entity object
   * @param $entity_id
   *   the eid to be set for the entity, if NULL and $entity dowes not have an eid, we will try to create one
   *
   ...
   * @param $object_in
   *   If there should be any object. The position of the object
   ...
   *   From where on the path should be generated in means
   ...
   * @param $mode
   *   defaults to 'field' - but may be 'group' or 'entity_reference' in special cases
   * @param $relative
   *   should it be relative to the other groups?
   * @param $variables
   *   the variable of index i will be set to the value of key i.
   *   The variable ?out will be set to the key "out".
   * @param $numbering
   *   the initial numbering for the results - typically starting with 0.
   *   But can be modified by this.
   */
  public function generateTriplesForPath($pb, $path, $primitiveValue = "", $subject_in = NULL, $object_in = NULL, $disambposition = 0, $startingposition = 0, $write = FALSE, $op = '=', $mode = 'field', $relative = TRUE, $variable_prefixes = [], $numbering = 0) {
    ...
    elseif ($has_primitive) {
      ...
      if (!empty($primitiveValue)) {
        ...
        if ($write) {
          ...
          elseif (strtoupper($op) == "LONGERTHAN" || strtoupper($op) == "SHORTERTHAN") {
            $regex = FALSE;
            $safe = TRUE;
          }
          // dpm($primitiveValue, "prim");.
   ...
   *
   */
  public function writeFieldValues($entity_id, array $field_values, $pathbuilder, $bundle_id = NULL, $old_values = [], $force_new = FALSE, $initial_write = FALSE) {
    // dpm($old_values, "ov");
    ...
    if (empty($init_entity)) {
      ...
      if ($force_new) {
        $entity = new WisskiEntity(['eid' => $entity_id, 'bundle' => $bundle_id], 'wisski_individual', $bundle_id);
    ...
    if (!empty($namespaces)) {
      ...
      \Drupal::logger('WissKI Ontology')->info(
        ...
        [
          'a' => $this->adapterId(),
          'n' => array_reduce(
            array_keys($namespaces), function ($carry, $k) use ($namespaces) {
      ...
      \Drupal::logger('WissKI Ontology')->info('Adapter {a}: no namespaces registered', ['a' => $this->adapterId()]);
    }
    ...
    if (stripos($format->getName(), 'xml') !== FALSE) {
      ...
      if (preg_match('/<(?:\w+:)?RDF[^>]*>/i', $file, $rdf_tag)) {
        ...
        foreach ($nsarray[0] as $ns_decl) {
          ...
          list($front, $back) = explode("=", substr($ns_decl, 5));
          $front = trim($front);
          // Chop the "/'.
  ...
  public function doTheReasoning() {
    ...
    $results = $this->directQuery(
        "SELECT ?class ?super WHERE { GRAPH ?g {"
    ...
    while (!empty($not_set)) {
      ...
      if (empty($invalid_supers)) {
        ...
        $new_domains = isset($domains[$prop]) ? $domains[$prop] : [];
        $new_ranges = isset($ranges[$prop]) ? $ranges[$prop] : [];
        // dpm($domains);
