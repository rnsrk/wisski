...
class WisskiEntityMatcher extends EntityMatcher {
  ...
  public function execute($string) {
    ...
    if (is_array($entities) && is_array($matches)) {
      \Drupal::logger("WissKI APUS")->info("linkit matches are {n2} with {n1} entities", ["n1" => count($entities), "n2" => count($matches)]);
    }
    else {
      \Drupal::logger("WissKI APUS")->info("linkit matches are {n2} with {n1} entities", ["n1" => count($entities), "n2" => $matches]);
    }
    return $suggestions;
  ...
  public function getMatches($string) {
    ...
    if ($string) {
      ...
      $results = db_select('wisski_title_n_grams', 'm')->fields('m', ['ent_num', 'bundle', 'ngram'])->condition('ngram', '%' . db_like($string) . '%', 'LIKE')->execute();
      $entities = [];
      while ($result = $results->fetchObject()) {
        $entities[$result->ent_num][$result->bundle] = $result->ngram;
      }
