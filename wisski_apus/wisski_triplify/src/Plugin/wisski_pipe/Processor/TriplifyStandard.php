...
class TriplifyStandard extends ProcessorBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['reference_properties']['default_property']['property'] = [
      '#type' => 'textfield',
    ...
    $form['reference_properties']['default_property']['inverse'] = [
      '#type' => 'checkbox',
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($ref_props['default_property']['property']) && !empty($ref_props['default_property']['property'])) {
      $this->reference_properties['default_property'] = [
  ...
  public function doRun() {
    ...
    // encapsulate: text may be xml/html fragment (leading/trailing chars or multiple root tags)
    $xhtml = "<div>$html_fragment</div>";
    $doc = new DOMDocument();
    ...
    if ($this->write_adapter !== NULL) {
      ...
      if ($graph_uri && $adapter) {
        $this->logInfo("Dropping text graph <{g}>", ['g' => $graph_uri]);
        $delete_query = "DROP GRAPH <$graph_uri>";
        dpm($delete_query);
        // $adapter->getEngine()->directUpdate($delete_query);
        $triple_str = join("\n  ", $triples);
        $this->logInfo("Inserting {c} triples into text graph <{g}>: {t}", ['g' => $graph_uri, 'c' => count($triples), 't' => $triple_str]);
  ...
  protected function triplifyAnnos($annos, $doc_inst) {
    ...
    foreach ($annos as $id => $anno) {
      ...
      if (!isset($anno->target) || empty($anno->target)) {
        continue;
      }
      ...
      $entity_infos = isset($anno->target->_entity_infos) ? $anno->target->_entity_infos : [];
      ...
      foreach ((array) $anno->target->ref as $ref_uri) {
        ...
        if (isset($entity_infos[$ref_uri])) {
          ...
          if (!$ref_uri) {
            dpm([$entity_id, AdapterHelper::getUrisForDrupalId($entity_id, $this->write_adapter), AdapterHelper::getUrisForDrupalId($entity_id)], 'lölö');
          }
  ...
  protected function getTextContent($e) {
    ...
    foreach ($e->childNodes as $c) {
      switch ($c->nodeType) {
        ...
        case XML_ELEMENT_NODE:
          ...
          if (in_array(strtolower($element->tagName), $ws_replacements)) {
            $text .= " $children_text ";
          }
          else {
            $text .= $children_text;
    ...
    // normalize: all whitespace as single blank.
   ...
   *
   */
  public function _wisski_texttrip_triplify_standard_walk($element, $doc_inst) {

    $triples = [];
    $uris_to_gids_and_rels = [];
    $agenda = [$element];
    ...
    while (!empty($agenda)) {
      ...
      if ($e->hasAttribute('data-wisski-anno-id')) {
        ...
        foreach (explode(' ', $e->getAttribute('class')) as $cl) {
          if (substr($cl, 0, 18) == 'wisski_anno_class_') {
            $gid = urldecode(substr($cl, 18));
          }
          if (substr($cl, 0, 16) == 'wisski_anno_uri_') {
            $uri = urldecode(substr($cl, 16));
          }
          if ($vocab_ctrl_exists && substr($cl, 0, 18) == 'wisski_anno_vocab_') {
            $voc = urldecode(substr($cl, 18));
          }
          if (substr($cl, 0, 20) == 'wisski_anno_approved') {
            $approved = TRUE;
          }
          if (substr($cl, 0, 19) == 'wisski_anno_subject') {
            $subject = TRUE;
          }
        ...
        if (!$approved && $settings['approved_only']) {
          continue;
        }

        if (!$uri && $gid == -1) {
          // We can't make anything useful with that.
          continue;
        }

        if (!$uri) {
          $uri = wisski_texttrip_create_instance_uri($gid);
        }

        // Can only be nonnull if vocab_ctrl module is enabled.
        if ($voc) {
        ...
        if ($voc) {
          ...
          if ($store->getErrors()) {
            foreach ($store->getErrors() as $e) {
              drupal_set_message("extracting triples: error asking TS: $e");
            }
          }
        ...
        if (!isset($uris_to_gids_and_rels[$uri])) {
          $uris_to_gids_and_rels[$uri] = ['gid' => -1, 'rels' => []];
        }
        // Cannot array_merge as keys get lost...
    ...
    foreach ($uris_to_gids_and_rels as $uri => $gid_and_rels) {
      ...
      foreach ($rels as $r => $targets) {
        ...
        while ($rel_path_data['group_id'] != $gid && $rel_path_data['group_id'] > 0) {
          ...
          if (!in_array($rel_path_data['id'], $interm_rels)) {
            $interm_rels[] = $rel_path_data['id'];
          }
        }
      }
      ...
      foreach ($terminal_rels as $rel => $targets) {
        foreach ($targets as $target) {
          ...
          if (!isset($uris_to_gids_and_rels[$target])) {
            // There is no annotation with the target uri.
            continue;
          }
          ...
          if (isset($uris_to_gids_and_rels[$target]['gid'])) {
            ...
            // Cannot determine group of target.
            continue;
          }
   ...
   *
   */
  public function _wisski_texttrip_triplify_standard_get_content($e) {

    $ws_replacements = ['br', 'p', '/p', 'div', '/div'];
    $text = '';
   ...
   *
   */
  public function _wisski_texttrip_triplify_standard_reference($settings, $uri, $gid, $text_inst, $text_inst_gid) {
   ...
   *
   */
  public function _wisski_texttrip_triplify_standard_path_link($pid, $uri1, $uri1_x, $uri2, $uri2_x) {

    $samepart = ($uri1_x == 0) ? [] : array_fill(0, $uri1_x * 2, '');
   ...
   *
   */
  public function _wisski_texttrip_triplify_standard_property_link($uri1, $prop, $uri2) {
   ...
   */
  public function _wisski_texttrip_triplify_standard_relation(&$group_sources, $rel, $target_uri = NULL, $target_class = NULL) {
    ...
    if ($target_class) {
      ...
      // Top group.
      if (!$tmp1['group_id']) {
        // This should be needless, as samapart of top group should be only first concept.
        $target_class = $tmp2['x0'];
      }
      else {
