...
class XHtml extends RunPipe {
  ...
  public function doRun() {
    ...
    if (isset($this->data->html)) {
      $this->data->xhtml = $this->data->html;
    }
    if (isset($this->data->text)) {
      $this->data->xhtml = $this->data->text;
    }
    if (!isset($this->data->xhtml)) {
      // There are no fields that we can work on.
      if ($logger != NULL) {
        $this->logWarning("There is no field for xhtml contents.");
      }
      return;
    }
    ...
    foreach ($this->data->annos as $k => $anno) {
      ...
      // + $start;.
      $e = $anno->range[1];
      $anno->plain_text_range = [$s, $e];
      if ($s == 0 && $e == 0) {
        // Pos 0 always shifts back to 0!
        continue;
      }
  ...
  protected function stripTags($default_lang = '') {
    ...
    // encapsulate: text may be xml/html snippet (leading/trailing chars or multiple root tags)
    $xhtml = "<div>$xhtml</div>";
    ...
    list($text, $textlen, $char_shifts, $lang_ranges, $annos) = $this->stripTagsWalk($doc->documentElement, $ws_replacements, '', 0, [], [], $default_lang, []);
  ...
  protected function stripTagsWalk($element, $replacements, $text, $textlen, $char_shifts, $langs, $cur_lang, $annos) {
    ...
    if ($element->hasAttribute('lang')) {
      $cur_lang = $element->getAttribute('lang');
    }
    if ($element->hasAttribute('xml:lang')) {
      $cur_lang = $element->getAttribute('xml:lang');
    }

    if (in_array(strtolower($element->tagName), $replacements)) {
      $text .= ' ';
      $langs[$cur_lang][] = [$textlen, $textlen + 1];
      $textlen += 1;
      $char_shifts[] = $textlen;
    }
    ...
    return [$text, $textlen, $char_shifts, $langs, $annos];
  ...
  protected function extractAnnotationFromTag($element) {
    ...
    if ($element->hasAttribute('class') && strpos($element->getAttribute('class'), 'wisski_anno') !== FALSE) {
      ...
      foreach (explode(' ', $element->getAttribute('class')) as $class) {
        if (substr($class, 0, 19) == 'wisski_anno_deleted') {
          $anno['deleted'] = TRUE;
        }
        if (substr($class, 0, 18) == 'wisski_anno_class_') {
          $anno['class'] = urldecode(substr($class, 18));
        }
        if (substr($class, 0, 16) == 'wisski_anno_uri_') {
          $anno['uri'] = urldecode(substr($class, 16));
        }
        if (substr($class, 0, 18) == 'wisski_anno_vocab_') {
          $anno['voc'] = urldecode(substr($class, 18));
        }
        if (substr($class, 0, 17) == 'wisski_anno_rank_') {
          $anno['rank'] = urldecode(substr($class, 17));
        }
        if (substr($class, 0, 20) == 'wisski_anno_new') {
          $anno['new'] = TRUE;
        }
        if (substr($class, 0, 20) == 'wisski_anno_proposed') {
          $anno['approved'] = FALSE;
        }
        if (substr($class, 0, 20) == 'wisski_anno_approved') {
          $anno['approved'] = TRUE;
        }
    ...
    elseif ($element->hasAttribute('data-wisski-anno')) {
      ...
      if ($element->hasAttribute('typeof')) {
        $anno['class'] = $element->getAttribute('typeof');
      }
      if ($element->hasAttribute('about')) {
        $anno['uri'] = $element->getAttribute('about');
      }
      if ($element->hasAttribute('data-wisski-anno-target')) {
        $anno['uri'] = $element->getAttribute('data-wisski-anno-target');
      }
      if ($element->getAttribute('data-wisski-anno-certainty') == 'approved') {
        $anno['approved'] = TRUE;
      }
    }
