...
class TestPage extends ControllerBase {
  ...
  public function ajax_analyse() {
    ...
    $json = ['ticket' => $ticket];
    if ($q->annos_only) {
      $json["annos"] = $text_struct['annos'];
    }
    else {
      $json["text_struct"] = $text_struct;
    }
  ...
  public function wisski_textanly_ajax_log() {
    ...
    // We use the textedit module to read the post parameters.
    $q = (object) ['ticket' => NULL, 'levels' => ['info']];
    ...
    if ($ticket === NULL) {
      ...
      print json_encode(["error" => "no ticket specified"]);
      return;
    }
