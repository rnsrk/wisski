  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($urls as $url) {
      ...
      elseif (mb_substr($url, 0, 7) !== 'http://' && mb_substr($url, 0, 8) !== 'https://') {
        drupal_set_message(t("Skipping malformed URI: %l. If you know it is right, enclose it in %b", ["%l" => $url, "%b" => "<>"]), 'error');
        continue;
      }
    ...
    if ($status === TRUE) {
      ...
      drupal_set_message($this->t('Could not merge entities: @e', ['@e' => $status]), 'error');
    }
  }
