...
class WisskiSkosEngine extends Sparql11Engine implements PathbuilderEngineInterface {
  ...
  public function getPathAlternatives($history = [], $future = [], $fast_mode = FALSE, $empty_uri = 'empty') {
    ...
    if (!empty($history)) {
      ...
      elseif ($this->isValidUri('<' . $candidate . '>')) {
        $last = $candidate;
        if ($this->isAProperty($last) === FALSE) {
      ...
      if ($candidate !== $empty_uri) {
        if ($this->isValidUri('<' . $candidate . '>')) {
          ...
          if ($search_properties === NULL) {
            if ($this->isAProperty($next) === FALSE) {
              $search_properties = TRUE;
            }
          }
  ...
  public function getPropertiesFromStore($class = NULL, $class_after = NULL, $fast_mode = FALSE) {
    ...
    if ($fast_mode) {
      ...
      if (isset($class_after)) {
        $query .= "?property rdfs:range <$class_after>.";
      }
    }
    else {
      if (isset($class)) {
  ...
  public function loadEntity($id) {
    ...
    foreach ($result as $thing) {
      ...
      // $uri = str_replace('/','\\',$uri);.
      $out = ['eid' => $id, 'bundle' => 'e21_person', 'name' => 'frizt'];
  ...
  public function loadMultipleEntities($ids = NULL) {
    ...
    foreach ($result as $thing) {
      ...
      $out[$uri] = ['eid' => $uri, 'bundle' => 'e21_person', 'name' => 'frizt'];
    }
      ...
      foreach ($entity_ids as $eid) {
        ...
        if (!empty($path)) {
          ...
          if ($pbarray['parent'] > 0 && $path->isGroup()) {
            ...
            // (count($this->getClearGroupArray($par, $pb))-1), $main_property, $path->getDisamb()));
            $out[$eid][$field_id] = array_merge($out[$eid][$field_id], $this->pathToReturnValue($path, $pb, $eid, count($path->getPathArray()) - count($clearPathArray), $main_property));
  ...
  public function writeFieldValues($entity_id, array $field_values, $bundle = NULL) {
    ...
    foreach ($pbs as $pb) {
      ...
      // @TODO !!!
      $old_values = $this->loadFieldValues([$entity_id], array_keys($field_values), $bundle);
