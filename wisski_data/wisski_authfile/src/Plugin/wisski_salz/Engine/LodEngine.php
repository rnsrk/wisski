...
class LodEngine extends NonWritableEngineBase implements PathbuilderEngineInterface {
  ...
  protected function substitute($substs, $strings) {
    ...
    foreach ($substs as $k => $v) {
      // The inner {} are bound to the variable $k due to phps var escape mechanism!
      $sic[] = "{{$k}}";
      $corr[] = $v;
    }
    foreach ($strs as &$str) {
      ...
      $str = join(
        '{', array_map(
            function ($a) use ($sic, $corr) {
                return strtr($a, array_combine($sic, $corr));
            }, explode('{{', $str)
        )
      );
    }
