...
interface EngineInterface extends PluginInspectionInterface, ConfigurablePluginInterface, PluginFormInterface {
   ...
   *
   * @param $uris
   *   an associative array where the keys are adapter_ids and the values are uris which all mean the same individuum
   *   the mapping denotes that the very adapter is holding information about that very URI
   * @param entity_id the drupal ID for the entity that all the uris from $uris
   *   identify. If NULL we just save the uri identification without drupal ID matching.
   */
   ...
   *
   * @param $entity_ids
   *   an array of the entity IDs.
   * @param $field_ids
   *   an array with the machine names of the fields to search the values for
   * @param $language
   *   language code for the desired translation
   *
   ...
   *
   * @param $field_id
   *   the machine name of the field to search the value for
   * @param $property_ids
   *   an array of specific sub-field property names e.g. value
   * @param $entity_ids
   *   an array of the entity IDs.
   * @param $language
   *   language code for the desired translation
   ...
   *
   * @param $conjunction
   *   thetype of condition conjunction used i.e. AND or OR
   *
   ...
   *
   * @param $entity_id
   *   the entity's drupal-internal ID
   * @param $field_values
   *   an array of field values keyed by the field_id. The second level arrays contain
   ...
   *   ]
   * @param $pathbuilder
   *   a pathbuilder object, that is connected to this adapter
   * @param $bundle
   *   the ID of the bundle the entities are in
   * @param $origignal_values
   *   an array of 'old' field values helping the adapter to decide which values to write, if necessary
   * @param $force_creation
   *   set to TRUE if the adapter shall store entity info even if it di not know the entity before
   * @param $initial_write
   *   set to TRUE if it is the first initial write of the entity after its creation. In this case we write everything without considering old values.
   *
