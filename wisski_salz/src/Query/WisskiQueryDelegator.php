...
class WisskiQueryDelegator extends WisskiQueryBase {
  ...
  public function execute() {
    ...
    if ($this->count) {
      ...
      if (!empty(self::$empties)) {
        $result -= count(self::$empties);
      }
      return $result;
    }
    else {
        ...
        if (!$is_sparql) {
          ...
          if ($pager || !empty($this->range)) {
            ...
            foreach ($this->dependent_queries as $query) {
              ...
              if (!empty(self::$empties)) {
                $result = array_diff($result, self::$empties);
              }
              return $result;
            }
          }
  ...
  protected function pagerQuery($limit, $offset) {
    ...
    while (!empty($query)) {
      ...
      if (!empty(self::$empties)) {
        $new_results = array_diff($new_results, self::$empties);
      }
  ...
  public function notExists($field, $langcode = NULL) {
    ...
    foreach ($this->dependent_queries as $query) {
      $query->notExists($field, $langcode);
    }
    return $this;
  }
  ...
  public function sort($field, $direction = 'ASC', $langcode = NULL) {
    ...
    foreach ($this->dependent_queries as $query) {
      $query->sort($field, $direction, $langcode);
    }
    return $this;
  }
