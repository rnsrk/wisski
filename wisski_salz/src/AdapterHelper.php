...
class AdapterHelper
{
     ...
     *                                                                                                                                   If no entity ID is provided AND none can be found in the data, we will create one
     *                                                                                                                                   for later use
    ...
    public static function setSameUris($uris,$entity_id=null) 
    {
        ...
        if (WISSKI_DEVEL) {
            \Drupal::logger("AH:ssu")->debug(
                "$entity_id and {uris}: {bt}", ["uris" => join(" ", $uris),"bt"=>join(
                    '//', array_map(
                        function ($a) {
                            return $a['function'];
        ...
        if(empty($entity_id)) {
            \Drupal::logger("AH:ssu")->debug(
                "No entity id could be detected for uris: {uris}: {bt}", ["uris" => join(" ", $uris),"bt"=>join(
                    '//', array_map(
                        function ($a) {
                            return $a['function'];
                        }, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 5)
    ...
    public static function doGetDrupalIdForUri($uri,$create_on_fail=true,$input_adapter_id=null) 
    {
        ...
        if (WISSKI_DEVEL) {
            \Drupal::logger("AH:difu")->debug(
                "$id and $uri and $input_adapter_id: {bt}", ["bt"=>join(
                    '//', array_map(
                        function ($a) {
                            return $a['function'];
    ...
    public static function getOnlyOneUriPerAdapterForDrupalId($eid, $adapter_id=null, $create = true) 
    {
        if (!is_numeric($eid)) {
            ...
            if (WISSKI_DEVEL) {
                ...
                \Drupal::logger('wisski salz')->warning(
                    __METHOD__ . ": Expected entity id, got URI: $eid: {bt}", ["bt"=>join(
                        '//', array_map(
                            function ($a) {
                                return (isset($a['class']) ? $a['class'] . '::' : '') . $a['function'];
                            }, debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 8)
                        )
                    )]
                );
