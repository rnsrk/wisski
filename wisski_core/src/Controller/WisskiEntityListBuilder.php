...
class WisskiEntityListBuilder extends EntityListBuilder {
  ...
  public function render($bundle = '', $entity = NULL) {
    ...
    if ($grid_type === 'table') {
      $header = ['preview_image' => $this->t('Entity'), 'title' => '', 'operations' => $this->t('Operations')];
    }
    if ($grid_type === 'grid') {
      $header = NULL;
    }
  ...
  private function getOperationLinks($entity_id) {
    ...
    $operations = [
      'view' => ['entity.wisski_individual.canonical', $this->t('View')],
      'edit' => ['entity.wisski_individual.edit_form', $this->t('Edit')],
      'delete' => ['entity.wisski_individual.delete_form', $this->t('Delete')],
    ];
    $i = 0;
    $links = [];
    foreach ($operations as $key => list($route, $label)) {
      $links[$key] = [
        'url' => Url::fromRoute($route, ['wisski_individual' => $entity_id, 'wisski_bundle' => $this->bundle->id()]),
        'weight' => $i++,
        'title' => $label,
      ];
    }
  ...
  private function getGridTypeBlock() {
    ...
    $block['table'] = [
      ...
      'link' => [
        '#type' => 'link',
        '#url' => Url::fromRoute('<current>', ['type' => 'table']),
    ...
    foreach (range(2, 10) as $width) {
      $ops[] = [
        'url' => Url::fromRoute('<current>', ['type' => 'grid', 'width' => $width]),
        'title' => $width,
      ];
    }
    $block['grid']['links'] = [
  ...
  public function buildRowForId($entity_id) {
    ...
    $entity_url = Url::fromRoute('entity.wisski_individual.canonical', ['wisski_bundle' => $this->bundle->id(), 'wisski_individual' => $entity_id]);
