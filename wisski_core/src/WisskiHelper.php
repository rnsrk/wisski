...
class WisskiHelper {
   ...
   * @param $offset
   *   the index where to split the array. The entry at $offset will be the first in the
   ...
   *
   * @param $array
   *   the array where the portion shall be inserted
   * @param $insertion
   *   the portion to be inserted
   * @param $offset
   *   the first index of the inserted sub-array after insertion
   *
  ...
  public static function array_insert(array $array, array $insertion, $offset = NULL) {
    ...
    if ($offset == 0) {
      return array_merge($insertion, $array);
    }
   ...
   *   the input array
   * @param $offset
   *   the first index to be removed
   * @param $the
   *   length of the portion to remove
   *
   ...
   * @param $fieldtype
   *   The type of the field or null for any field
   * @param $field_format
   *   The format of the field or null for any
   */
  public static function getFieldsForBundleId($bundleid, $fieldtype = NULL, $field_format = NULL, $recursive = TRUE) {
    ...
    foreach ($field_definitions as $key => $field_definition) {
      if (!($field_definition instanceof BaseFieldDefinition)) {
        ...
        elseif ($fieldtype == $field_definition->getType()) {
    ...
    if ($recursive) {
      foreach ($subbundles as $subbundle) {
        $out = array_merge($out, WisskiHelper::getFieldsForBundleId($subbundle, $fieldtype, $field_format, $recursive));
      }
    }
