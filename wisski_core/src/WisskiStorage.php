...
class WisskiStorage extends ContentEntityStorageBase implements WisskiStorageInterface {
  ...
  private $image_style;
  private $adapter;
  private $preview_image_adapters = [];
   ...
   *
   */
  public function getCacheValues($ids, $field_id = [], $bundle_id = []) {

    foreach ($ids as $id) {
      $cached_field_values = db_select('wisski_entity_field_properties', 'f')
        ->fields('f', ['fid', 'ident', 'delta', 'properties'])
        ->condition('eid', $id);
   ...
   *   entity ID
   * @param $cached
   *   TRUE for static caching, FALSE for forced update
   *
  ...
  protected function getEntityInfo(array $ids, $cached = FALSE) {
    ...
    elseif (!empty($this->stored_entities)) {
    ...
    foreach ($ids as $id) {
      ...
      foreach ($adapters as $aid => $adapter) {
        ...
        if ($adapter->hasEntity($id)) {
          ...
          // HACK! (randomly takes the first one.
            ...
            try {
              ...
              foreach ($field_definitions as $field_name => $field_def) {
                ...
                if ($field_def instanceof BaseFieldDefinition) {
                  ...
                  if ($field_name === 'name') {
                    $new_field_values[$id][$field_name] = [wisski_core_generate_title($id)];
                  }

                  // This is a base field and cannot have multiple values.
                  // @TODO make sure, we load the RIGHT value
                  if (empty($new_field_values)) {
                    $new_field_values = $adapter->loadPropertyValuesForField($field_name, [], [$id], $bundleid);
                  }
                ...
                if ($field_def->getType() === 'entity_reference') {
                  ...
                  elseif (count($target_bundles) === 1) {
                    drupal_set_message($this->t('There is no target bundle id for field %field - I could not continue.', ['%field' => $field_name]));
                  }
                  else {
                    drupal_set_message($this->t('Multiple target bundles for field %field, %field_label', ['%field' => $field_def->getLabel(), '%field_label' => $field_name]));
                    // dpm($target_bundles);
                ...
                if ($hack_type == 'text_with_summary' || $hack_type == 'text_long') {
                  $formats = \Drupal::entityManager()->getStorage('filter_format')->loadByProperties(['status' => TRUE]);
                ...
                if ($is_file && $has_values) {
                  ...
                  foreach ($new_field_values[$id][$field_name] as $key => &$properties_array) {
                    ...
                    if (!isset($properties_array['original_target_id']) && !isset($properties_array['target_id'])) {
                      continue;
                    }
                    elseif (isset($properties_array['target_id']) && !isset($properties_array['original_target_id'])) {
                      $properties_array['original_target_id'] = $properties_array['target_id'];
                    }
                ...
                if (isset($new_field_values[$id][$field_name])) {
                  $cached_field_values = db_select('wisski_entity_field_properties', 'f')
                    ->fields('f', ['ident', 'delta', 'properties'])
                    ->condition('eid', $id)
                  ...
                  if (!isset($info[$id]) || !isset($info[$id][$field_name])) {
                    $info[$id][$field_name] = $new_field_values[$id][$field_name];
                  }
                  else {
                    $info[$id][$field_name] = array_merge($info[$id][$field_name], $new_field_values[$id][$field_name]);
                  }
                }
              }
            }
  ...
  public function getFileId($file_uri, &$local_file_uri = '', $entity_id = 0) {
    ...
    if (empty($file_ids)) {
      ...
      if (file_destination($local_file_uri, FILE_EXISTS_ERROR) === FALSE) {
        ...
        try {
          ...
          if (empty($data)) {
            drupal_set_message($this->t('Could not fetch file with uri %uri.', ['%uri' => $file_uri]), 'error');
          }
          ...
          drupal_set_message($this->t('Could not create file with uri %uri. Exception Message: %message', ['%uri' => $file_uri, '%message' => $e->getMessage()]), 'error');
        }
      }
    ...
    // dpm($local_file_uri, "loc");
    // set cache.
    \Drupal::cache()->set($cid, [$value, $local_file_uri]);
    return $value;
  }
  ...
  public function getPublicUrlFromFileId($file_id) {
    ...
    if ($file_object = File::load($file_id)) {
      return str_replace(
      // Standard file uri is public://.../filename.jpg.
        'public:/',
      // We want DRUPALHOME/sites/default/.../filename.jpg.
  ...
  protected function doSaveFieldItems(ContentEntityInterface $entity, array $names = []) {
    ...
    if (isset($uid) && empty($uid->getValue()['target_id'])) {
      $user = \Drupal::currentUser();
      // dpm($values, "before");.
      $uid->setValue(['target_id' => (int) $user->id()]);
    }
    ...
    foreach ($pbs_info as $pbid => $info) {
      ...
      elseif ($info['preferred_local']) {
        ...
        drupal_set_message(t('The preferred local store %a is not writable.', ['%a' => $adapter->label()]), 'warning');
      }
    }
    ...
    if ($bundle) {
      $bundle->flushTitleCache($entity_id);
    }
  ...
  protected function doDeleteFieldItems($entities) {
    ...
    $local_adapters = [];
    $writable_adapters = [];
    // Adapters that we use for deleting the entities.
    $delete_adapters = [];
    ...
    if ($diff = array_diff_key($local_adapters, $writable_adapters)) {
      if (count($diff) === 1) {
        ...
        drupal_set_message('The preferred local stores ' . implode(', ', array_keys($diff)) . ' are not writable', 'warning');
      }
    }
  ...
  public function preparePreviewImages() {
    ...
    if (!$pref_local) {
      ...
      if (empty($this->preview_image_adapters)) {
        $this->preview_image_adapters = [$pref_local];
      }
    }
    return TRUE;
  }
  ...
  public function getPreviewImageUri($entity_id, $bundle_id) {
    ...
    foreach ($this->preview_image_adapters as $adapter_id => $adapter) {
      ...
      if (count($images) > 1) {
        ...
        foreach ($images as $image) {
          ...
          while (!$found_weight && $old_to_look_for != $to_look_for) {
            ...
            // Get the weight.
            $cached_field_values = db_select('wisski_entity_field_properties', 'f')
              ->fields('f', ['eid', 'fid', 'bid', 'ident', 'delta', 'properties']);
            // ->condition('eid',$id)
            // ->condition('bid',$values[$id]['bundle'])
        ...
        $images = [current(array_keys($deltas))];
      ...
      if (empty($images)) {
        if (WISSKI_DEVEL) {
          \Drupal::logger('wisski_preview_image')->debug('No preview images available from adapter ' . $adapter->id());
        }
        continue;
      }
      ...
      if (WISSKI_DEVEL) {
        \Drupal::logger('wisski_preview_image')->debug("Images from adapter $adapter_id: " . serialize($images));
      }
  ...
  private function getPreviewStyle() {
    ...
    if (is_null($image_style)) {
      ...
      $values = ['name' => $image_style_name, 'label' => 'Wisski Preview Image Style'];
