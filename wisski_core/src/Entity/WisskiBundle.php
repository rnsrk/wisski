...
*/
  ...
  public static function getWissKIMenus() {
    return [
      'navigate' => 'entity.wisski_bundle.entity_list',
  ...
  public function getTitlePattern() {
    ...
    if (empty($this->title_pattern)) {
      ...
      $state = \Drupal::state()->get('wisski_core_title_patterns') ?: serialize([]);
  ...
  private function applyTitlePattern($pattern, $entity) {
    ...
    $max = ($count * ($count + 1)) / 2;
    $count = 0;
    // && list($key,$attributes) = each($pattern)) {.
    while ($count < $max && current($pattern)) {
      ...
      if ($attributes['type'] === 'path') {
        ...
        switch ($name) {
          ...
          case 'uri.long':
          case 'uri.short':
            $values = [$this->getUriString($entity_id, $name)];
            break;
  ...
  public function createFallbackTitle($entity_id) {
    ...
    switch ($this->onEmpty()) {
      case self::FALLBACK_TITLE:
        return $this->fallback_title;

      case self::DEFAULT_PATTERN:
        return $this->applyTitlePattern($this->getDefaultPattern(), $entity_id);
      ...
      if ($pb->hasPbPath($path_id)) {
        ...
        if (AdapterHelper::getUrisForDrupalId($eid, $adapter->id(), FALSE)) {
          ...
          if ($pbpath['bundle'] == $pbpath['field'] || $pbpath['fieldtype'] == "entity_reference") {
            ...
            if (WISSKI_DEVEL) {
              \Drupal::logger($pb_id . ' ' . $path_id . ' ' . __FUNCTION__)->debug('Entity ' . $eid . "{out}", ['out' => serialize($new_values)]);
            }
          }
        }
        if (empty($new_values)) {
   ...
   *
   * @param $entity_ids
   *   an array of IDs of entities whose titles shall be removed from this bundle's cache list, if NULL, all titles will be deleted
   */
  ...
  public function getCachedTitle($entity_id) {
    ...
    if (!isset($this->cached_titles[$entity_id])) {
      if ($title = WisskiCacheHelper::getEntityTitle($entity_id, $this->id())) {
        $this->cached_titles[$entity_id] = $title;
      }
      else {
        return NULL;
  ...
  public function setOnEmpty($type) {
    ...
    if ($type == self::DEFAULT_PATTERN || $type == self::FALLBACK_TITLE || $type == self::DONT_SHOW) {
      ...
      drupal_set_message('Invalid fallback type for title pattern');
    }
  }
   ...
   *
   */
  public function deleteBundleFromMenu($menu_name, $destination_route = "entity.wisski_bundle.entity_list", $parameters = []) {

    if (empty($parameters)) {
      $parameters = ["wisski_bundle" => $this->id()];
    }
  ...
  public function addViewForBundle($menu_name, $weight, $enabled) {
    ...
    $options['display']['default'] = [
      ...
      'position' => 0,
      'display_options' => [
        'access' => ['type' => 'perm', 'options' => ['perm' => 'view any wisski content']],
    // array('type' => 'tag', 'options' => array() ),.
        'cache' => ['type' => 'none'],
        ...
        'pager' => [
          ...
          'options' => [
            ...
            'id' => 0,
            'total_pages' => NULL,
            'tags' => ['previous' => '<<', 'next' => '>>', 'first' => '<< First', 'last' => 'Last >>'],
            'expose' => [
   ...
   */
  public function addBundleToMenu($menu_name, $destination_route = "entity.wisski_bundle.entity_list", $parameters = []) {
