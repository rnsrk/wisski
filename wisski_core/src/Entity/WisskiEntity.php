 ...
 */
    ...
    if ($use_status) {
      $fields['status']->setDisplayOptions(
        'form', [
  ...
  public function tellMe() {
    ...
    foreach ($keys as $key) {
      ...
      $return[$key] = ['key' => $key, 'field_name' => $field_name, 'property' => $property, 'value' => $value];
    }
    return $return;
  }
  ...
  public function getValues($storage, $save_field_properties = FALSE) {
    ...
    return [$this->extractFieldData($storage, $save_field_properties), $this->original_values];
  }
  ...
  protected function extractFieldData($storage, $save_field_properties = FALSE) {
    ...
    if ($save_field_properties) {
      ...
      $query = db_insert('wisski_entity_field_properties')
        ->fields(['eid', 'bid', 'fid', 'delta', 'ident', 'properties']);
