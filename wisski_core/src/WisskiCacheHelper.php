...
class WisskiCacheHelper {
   ...
   *
   */
  public static function putCacheData($cid, $data, $tags = NULL) {
    if (is_null($tags)) {
   ...
   *
   */
  public static function getCacheData($cid) {
   ...
   *
   */
  public static function flushCacheData($cid) {
   ...
   *
   */
  public static function putEntityTitle($entity_id, $entity_title, $bundle_id = NULL) {
    ...
    db_insert('wisski_title_n_grams')->fields(
        [
          'ent_num' => $entity_id,
   ...
   *
   */
  public static function getEntityTitle($entity_id, $bundle_id = NULL) {
   ...
   *
   */
  public static function flushEntityTitle($entity_id, $bundle_id = NULL) {
   ...
   *
   */
  public static function flushAllEntityTitles($bundle_id = NULL) {
   ...
   *
   */
  public static function getEntitiesWithEmptyTitle($bundle_id = NULL) {

    $empties = ['', 'NULL', 'FALSE'];
    $query = db_select('wisski_title_n_grams', 'n')->fields('n', ['ent_num'])->condition('ngram', $empties, 'IN');
    if (!is_null($bundle_id)) {
   ...
   *
   */
  public static function putCallingBundle($entity_id, $bundle_id) {
    ...
    if ($result = $query->fetch()) {
      if ($result->bid !== $bundle_id) {
        $db->update('wisski_calling_bundles')->fields(['bid' => $bundle_id])->condition('eid', $entity_id)->execute();
      }
    }
    else {
      $db->insert('wisski_calling_bundles')->fields(['eid' => $entity_id, 'bid' => $bundle_id])->execute();
    }
  }
   ...
   *
   */
  public static function getCallingBundle($entity_id) {
   ...
   *
   */
  public static function flushCallingBundle($entity_id) {
 ...
 */
   ...
   *
   */
  public static function preparePreviewImages(array $entity_ids) {

    if (empty($entity_ids)) {
      self::$gathered_preview_images = [];
    }
    else {
      $query = db_select('wisski_preview_images', 'p')
        ->fields('p', ['eid', 'image_uri'])
   ...
   *
   */
  public static function putPreviewImageUri($entity_id, $preview_image_uri) {
    ...
    db_insert('wisski_preview_images')->fields(['eid' => $entity_id, 'image_uri' => $preview_image_uri])->execute();
  }

  /**
   *
   */
  public static function getPreviewImageUri($entity_id) {
    ...
    if (isset(self::$gathered_preview_images)) {
      ...
      $query = db_select('wisski_preview_images', 'p')->fields('p', ['image_uri'])->condition('eid', $entity_id)->execute();
   ...
   *
   */
  public static function flushPreviewImageUri($entity_id) {
   ...
   *
   */
  public static function flushAllPreviewImageUris() {
