...
class WisskiIndividualQuery extends QueryPluginBase {
   ...
   *
   * @var \Drupal\Core\Entity\Query\QueryInterface\Drupal\wisski_salz\Query\WissKIQueryDelegatorinourcase
   */
  public $query;
   ...
   * ResultRow objects.
   *
   * @var arraykeysandvaluesarethefieldIDs
   */
  public $fields = [];
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    // TODO: do we need to set it here if pager is only inited in this->build()?
   ...
   */
  public function addField($base_table, $base_field) {
   ...
   *
   */
  protected function fillResultValues($entity_ids, $bundle_ids = [], $filter_regex = []) {
    // dpm($bundle_ids, "this");.
    $eid_to_uri_per_aid = [];
    ...
    while (($field = array_shift($fields)) !== NULL) {
      ...
      elseif ($field == 'bundle' || $field == 'bundle_label' || $field == 'bundles') {
        ...
        foreach ($values_per_row as $eid => &$row) {
          ...
          $row['bundles'] = $bids;
          // TODO: make a more sophisticated choice rather than the first one.
          $bid = reset($bids);
          $row['bundle'] = $bid;
      ...
      elseif (strpos($field, "wisski_path_") === 0 && strpos($field, "__") !== FALSE) {
        ...
        if (count($pb_and_path) != 2) {
          ...
          elseif (!$path) {
            ...
            // ->getFieldDefinitions('wisski_individual',$values_per_row[$eid]['bundle']);.
            $field_def = \Drupal::service('entity_field.manager')->getFieldMap();
              ...
              if (!$adapter) {
                ...
                if (!($engine instanceof Sparql11EngineWithPB)) {
                  ...
                  if ($is_reference) {
                    ...
                    if ($disamb < 2) {
                      $disamb = count($path->getPathArray());
                    }
                  ...
                  // Keep for reverse mapping of results.
                  $uris_to_eids = [];
                  ...
                  foreach ($result as $sparql_row) {
                    if (isset($uris_to_eids[$sparql_row->x0->getUri()])) {
                      ...
                      elseif ($is_reference) {
                        ...
                        $values_per_row[$eid][$field][] = ['value' => $referenced_title, 'target_id' => $referenced_eid, 'wisskiDisamb' => $referenced_uri];
                        // Duplicate the information to the field for the entity-management.
                        $values_per_row[$eid][$field_to_check][] = ['value' => $referenced_title, 'target_id' => $referenced_eid, 'wisskiDisamb' => $referenced_uri];
                        ...
                        if (!empty($disamb)) {
                          ...
                          // } else {.
                          $values_per_row[$eid][$field][] = [$main_prop => $sparql_row->$out_prop->getValue(), 'wisskiDisamb' => $sparql_row->$disamb->getUri()];
                          $values_per_row[$eid][$field_to_check][] = [$main_prop => $sparql_row->$out_prop->getValue(), 'wisskiDisamb' => $sparql_row->$disamb->getUri()];
                          // }.
                        }
                        else {
                          ...
                          if (!empty($is_file)) {
                            $storage = \Drupal::entityTypeManager()->getStorage('wisski_individual');
                            $val = $storage->getFileId($sparql_row->$out_prop->getValue());
                            $values_per_row[$eid][$field][] = [$main_prop => $val];
                            $values_per_row[$eid][$field_to_check][] = [$main_prop => $val];
                          }
                          else {
                            $values_per_row[$eid][$field][] = [$main_prop => $sparql_row->$out_prop->getValue()];
                            $values_per_row[$eid][$field_to_check][] = [$main_prop => $sparql_row->$out_prop->getValue()];
                          }
                        }
                      }
    ...
    if ($do_dummy_load) {
      foreach ($values_per_row as $lkey => &$row) {
        ...
        if (!isset($pseudo_entity_fields[$lkey]['eid'])) {
          $pseudo_entity_fields[$lkey]['eid'] = ['value' => $lkey];
        }
        // dpm($row);
        // $row['_entity'] = entity_load('wisski_individual', $row['eid']);;
        // $bid = reset($bundle_ids);
        ...
        $entities = \Drupal::entityManager()->getStorage('wisski_individual')->addCacheValues([$lkey => $lkey], $pseudo_entity_fields);
        // foreach($row as $field_name => $data) {
        // $entities[$lkey]->$field_name = $data;
        // }
        // dpm(serialize($entities), "ent");
        // \Drupal::entityManager()->getStorage('wisski_individual')->addCacheValues(array($values_per_row[$eid]), $values_per_row);.
        $row['_entity'] = $entities[$lkey];
        // $row['_entity'] = entity_load('wisski_individual', $row['eid']);
