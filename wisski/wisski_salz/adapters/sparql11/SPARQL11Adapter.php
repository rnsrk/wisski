...
//    dpm(func_get_args(),__FUNCTION__);
    ...
    // if there is no starting concept - die.
    ...
    // if there are not any uris in the settings or if there are uris, 
    ...
    foreach($paths as $key => $path) {
      ...
      // @TODO this seems to be a little bit optimistic - shouldn't we check for numeric?
...
//          $query .= " ?$count rdf:type/rdfs:subClassOf* ".array_shift($path_array).". ";
        ...
        if(isset($settings['count'])) {
          $query .= " BIND(?p".$i."c" . $settings['count'] . " AS ?cnt)";
        }
